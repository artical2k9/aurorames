...
import com.mikemes.iam.exception.DuplicateEmailException;
...
public class KeycloakAdminClient {
    ...
    private static final Pattern MIKEMES_ROLE = Pattern.compile("^[A-Z][A-Z0-9_]{0,99}$");
    ...
    public List<String> getUserRoles(String userId) {
        return keycloak.realm(realm).users().get(userId)
                ...
                .filter(name -> MIKEMES_ROLE.matcher(name).matches())
                .toList();
    }
    ...
    public void setUserRoles(String userId, List<String> roleNames) {
        ...
        List<RoleRepresentation> currentMikeMES = current.stream()
                .filter(r -> MIKEMES_ROLE.matcher(r.getName()).matches())
                .toList();

        Set<String> desiredNames = new HashSet<>(roleNames);
        List<RoleRepresentation> toRemove = currentMikeMES.stream()
        ...
        Set<String> currentNames = currentMikeMES.stream()
                .map(RoleRepresentation::getName)
