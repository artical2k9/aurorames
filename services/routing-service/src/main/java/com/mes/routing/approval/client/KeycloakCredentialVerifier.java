...
/**
 * Verifies an approver's password against Keycloak using the Resource Owner Password Credentials
 * (Direct Access Grant) flow on the {@code mes-signature-verify} confidential client. The token KC
 * returns on success is never read or stored — only the HTTP outcome matters. Mirrors the
 * engineering-service e-signature verifier (research.md R1).
 */
@Component
