...
- **FR-046**: The Make / Buy Code field on the Item Master create/edit form MUST render as a **PrimeNG `p-selectbutton`** with two options: "Make" and "Buy". It MUST NOT be a dropdown `p-select`. The current three-value enum (MAKE / BUY / EITHER) is reduced to two options in the UI; "EITHER" is not selectable via the form (it may only be set programmatically or via API).
