...
## ERR-MES-089 — Angular CI ran lint only; template type error compiled clean through lint+tests but broke `ng build`
**Date:** 2026-06-14  **Category:** CI — Frontend / Type-Checking  **Promoted:** 2026-06-15
**Symptom:** A breadcrumb on `employee-detail.component.ts` was set with `{ label: 'Employees', url: '/labour/employees' }`, but the `Crumb` type is `{ label: string; route?: string[] }` — `url` is not a valid property. ESLint passed, the Vitest suite passed, and the code merged to Develop. The error only surfaced when a later branch ran `npm run build` locally: `TS2353: Object literal may only specify known properties, and 'url' does not exist in type 'Crumb'`.
**Root cause:** The Angular CI job (`angular` in `.github/workflows/ci.yml`) ran `npm run lint` and nothing else — no `ng build`, and not even `npm test`. ESLint does not perform Angular template/type analysis, so a binding to a non-existent property is invisible to it. The full template type-checker only runs during `ng build` (and `ng test`'s compile). With neither in CI, any type error that lint misses reaches Develop and silently breaks the production build for every subsequent branch until someone happens to build locally.
**Fix applied:** Added a `Build (type-check)` step (`npm run build`) to the `angular` CI job immediately after lint. Confirmed Develop builds clean before adding the gate so it does not block existing PRs. The job `name:` was left unchanged (`Angular — Lint`) to avoid breaking required-status-check matching in branch protection. The original `url`→`route` typo was fixed in PR #53.
**Rule:** A frontend CI gate that runs only `npm run lint` is insufficient — lint does not type-check templates. Every frontend CI pipeline must also run `npm run build` (the Angular compiler's full template type-checker). When adding a new CI type-check/build gate, first run it against the integration branch to confirm it is currently green, so the new gate does not retroactively block unrelated PRs. Separately noted gap: the Vitest suite is still not run in CI (only locally) — running `npm test` in CI is a recommended follow-up so test regressions are caught before merge, not just locally.

## ERR-MES-087 — PostgreSQL `DISTINCT ON` without tiebreaker returns indeterminate row when group values are equal
**Date:** 2026-06-12  **Category:** Backend — PostgreSQL / SQL  **Promoted:** 2026-06-12
**Symptom:** BOM browser showed revision 0 (the oldest) instead of revision 3 (the latest). After deploying a status-priority fix (DRAFT→1, APPROVED→3) and confirming via `unzip -p app.jar | strings | grep` that the correct SQL was in the running JAR, the UI still showed the wrong revision. The raw DB query also returned rev 0.
**Root cause:** `DISTINCT ON (bom_id) ... ORDER BY bom_id, CASE revision_status WHEN 'DRAFT' THEN 1 ... WHEN 'APPROVED' THEN 3 END` had no secondary tiebreaker. All 4 revisions had status APPROVED, so all produced CASE value 3. PostgreSQL's `DISTINCT ON` resolves ties by physical heap/storage order — which happened to be insertion order — so it picked revision 0 every time. This is non-deterministic: the result depends on vacuuming, MVCC row versions, and table bloat. The initial fix was correct for mixed-status scenarios but left the same-status scenario completely unaddressed.
**Fix applied:** Added `revision DESC` as the final sort key in both the main query and the count query of `BomRevisionRepository.findDisplayRevisionSummaries`. Verified correct result in `psql` before rebuilding.
**Rule:** Every `DISTINCT ON (col)` query must end with a deterministic tiebreaker after all CASE/grouping expressions. For revision tables: `ORDER BY col, CASE revision_status WHEN ... END, revision DESC`. For general records: append `created_at DESC` or `id DESC`. **Mandatory verification**: run the raw SQL in `psql` and confirm the expected row is returned — including the same-status edge case — before triggering a Docker build.

## ERR-MES-081 — Stale gateway container missing security config change; 401 on newly-permitted public endpoint
**Date:** 2026-06-10  **Category:** Deployment — Docker / Security Config  **Promoted:** 2026-06-12
**Symptom:** `POST /api/iam/auth/change-temporary-password` returned 401 for unauthenticated requests. The auth interceptor caught the 401, attempted `refreshToken()` with a null refresh token (no session existed), received 400 from KC, then called `endSession()` which silently redirected the user to the login page — hiding the real cause.
**Root cause:** The gateway Docker image was built on Jun 6. The commit that added `"/api/iam/auth/**"` to `permitAll()` in `gateway-service/SecurityConfig.java` landed on Jun 7. The running container was stale and blocked all pre-auth requests to the IAM public endpoints. Compounded by the auth interceptor swallowing the 401 and calling `endSession()` rather than propagating the error.
**Fix applied:** Copied the latest gateway JAR into the container via `docker cp` and restarted. Added a null-guard in `auth.interceptor.ts`: if `getRefreshToken()` is null, propagate the 401 to the caller instead of triggering `endSession()`.
**Rule:** After any commit that changes a gateway or service security config (permitAll rules, filter chain order, CORS, JWT validation), rebuild the affected Docker image before testing. Do not rely on a running container being current — check `docker images <name> --format "{{.CreatedAt}}"` against the commit date of the security-config file.

## ERR-MES-080 — JUnit 5 inherits @Test methods from parent IT class; shared DB causes 409 pollution
**Date:** 2026-06-10  **Category:** Testing — JUnit 5 / Integration  **Promoted:** 2026-06-10
**Symptom:** CI reported 12 failures in `ItemRevisionWorkflowIT`, all 409 CONFLICT. The 6 own tests and all 6 inherited tests from `ItemMasterControllerIT` failed because the part numbers (PART-001, etc.) were already inserted when `ItemMasterControllerIT` ran first in the same Testcontainers DB context.
**Root cause:** JUnit 5 inherits `@Test` methods from superclasses. `ItemRevisionWorkflowIT extends ItemMasterControllerIT` caused 6 parent tests to re-run in the subclass context, duplicating all part-number POST requests. The Testcontainers PostgreSQL container is shared (no rollback between classes). The same pattern had been fixed for `BomRevisionIT` and `ItemRevisionIT` in a prior pass but the rule was never written down, so the next new IT class repeated it.
**Fix applied:** Changed `ItemRevisionWorkflowIT extends ItemMasterControllerIT` → `ItemRevisionWorkflowIT extends BaseIntegrationTest`. Added inline helpers with the same implementation.
**Rule:** IT classes must never extend another concrete IT class that contains `@Test` methods. All must extend `BaseIntegrationTest` directly. Pre-PR check: `grep -rn "extends.*IT\b" services/ --include="*IT.java"` — every match is a violation.

## ERR-MES-075 — New module privileges not auto-granted to SYSTEM_ADMIN; 403 on first use
**Date:** 2026-06-08  **Category:** Backend — IAM / Privilege System  **Promoted:** 2026-06-08
**Symptom:** `GET /api/platform/uom` returned HTTP 403 `insufficient_scope` immediately after deployment. The platform-service startup log showed `platform privileges registered with IAM service`, yet no token included the new privileges.
**Root cause:** `PrivilegeRegistryClient.register()` → `PrivilegeService.registerManifest()` upserts `iam.privilege` rows only. It never touches `iam.role_privilege`. New privilege keys are catalogued but granted to nobody, including `SYSTEM_ADMIN`. No checklist item enforced the post-registration grant step.
**Fix applied:** `PrivilegeService.registerManifest()` now auto-grants every registered privilege to `SYSTEM_ADMIN` if not already held. Existing grants are loaded once before the item loop (avoids N+1). Idempotent: re-registering an existing key re-grants it if accidentally revoked. `AuditorAware` fills `granted_by = "system"` for the unauthenticated internal endpoint.
**Rule:** `registerManifest()` auto-grants to SYSTEM_ADMIN — no separate step needed when shipping new `@RequiresPrivilege` keys. For all other roles, new privileges require explicit assignment via the Role management UI. Pre-existing deployments self-heal on next service restart.
