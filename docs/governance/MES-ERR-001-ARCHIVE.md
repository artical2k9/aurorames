...
**Fix applied:** Added a `Build (type-check)` step (`npm run build`) and a `Unit tests` step (`npm test`) to the `angular` CI job immediately after lint. Confirmed Develop builds clean and the suite passes before adding the gates so they do not block existing PRs. The job `name:` was left unchanged (`Angular — Lint`) to avoid breaking required-status-check matching in branch protection. The original `url`→`route` typo was fixed in PR #53.
**Rule:** A frontend CI gate that runs only `npm run lint` is insufficient — lint does not type-check templates and does not run tests. Every frontend CI pipeline must also run `npm run build` (the Angular compiler's full template type-checker) **and** `npm test` (Vitest) so type errors and test regressions are caught before merge, not just locally. When adding a new CI build/type-check/test gate, first run it against the integration branch to confirm it is currently green, so the new gate does not retroactively block unrelated PRs.
