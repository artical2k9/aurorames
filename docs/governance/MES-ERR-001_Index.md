...
| ERR-MES-081 | 2026-06-10 | Deployment — Docker / Security & Routes | Stale gateway image missed later security-config AND route changes (404 on new `/api/v1/labour/**` etc.); rebuild image after any gateway security/route commit (`up -d` reuses image without `--build`); unauthenticated 401 ≠ route exists (security runs before routing) — verify new routes with a real token |
| ERR-MES-082 | 2026-06-13 | Frontend — Testing / Contract Drift | Adding a required field to a shared DTO (or new route-param/service-method/precondition in a component) without updating sibling `*.spec.ts` mocks leaves the whole `ng test` build red; Angular test build is all-or-nothing |
| ERR-MES-083 | 2026-06-13 | Backend — Static Analysis / Serializable | `transient` on a data-carrying collection field of a Serializable exception trips SpotBugs SE_TRANSIENT_FIELD_NOT_RESTORED; use `List.copyOf()` and leave the field non-transient |
| ERR-MES-084 | 2026-06-13 | Testing — Behaviour Change | Adding a precondition gate to an existing endpoint silently breaks existing happy-path ITs; grep all test call sites of the method and update fixtures in the same commit |
| ERR-MES-085 | 2026-06-14 | Deployment — Docker / DB Provisioning | New service's `*_DB_USER`/`*_DB_PASSWORD` must go in the **postgres** service env block (init script reads them there; missing → `:-changeme` fallback → `password authentication failed` crash-loop); one self-contained `DO`/`END IF`/`END` block + own `ALTER DEFAULT PRIVILEGES` per role; verify on a fresh volume |
| ERR-MES-086 | 2026-06-14 | Deployment — Docker / Dependency Gating | Never gate a functional service on a monitoring-dashboard healthcheck (`depends_on: admin-service: service_healthy`); platform-service stayed `Created` → grid-preference API 500 → column picker silently broke everywhere. Gate only on real deps (DB/KC/Kafka). OAuth2 **login** clients can't use `issuer-uri` discovery under KC split-horizon — use explicit endpoints |
| ERR-MES-087 | 2026-06-12 | Backend — PostgreSQL / SQL | `DISTINCT ON` without `revision DESC` tiebreaker returns indeterminate (oldest) row when all statuses equal; always add `revision DESC` as final sort key |
| ERR-MES-088 | 2026-06-14 | Testing — Integration / Pagination | New ITs adding rows to the shared Testcontainers DB break existing list tests that filter AFTER pagination (matching row pushed off page 0); run the FULL service `check` before raising, and make filtered-list assertions request an explicit large page size |
| ERR-MES-089 | 2026-06-14 | CI — Frontend / Type-Checking | Angular CI ran `npm run lint` only — no `ng build`/`npm test`; a template binding to a non-existent type property (`Crumb.url`) passed lint+tests and broke the production build on Develop. Every frontend CI pipeline must also run `npm run build` (Angular template type-checker); confirm the integration branch is green before adding a new gate |
