...
**Rule:** When implementing against an external service (Keycloak, Kafka, Postgres) where: (a) the version is ≥ 1 major version newer than the last well-known stable, OR (b) the feature involves a documented breaking change (e.g. KC25 User Profile, KC26 required actions), STOP before committing and explicitly flag to the user: "This relies on [X] behavior in version [Y]. My training data on this specific change may be shallow. Recommend verifying against the actual container before treating the CI run as the first test." Do not spend CI runs discovering what a 2-minute local smoke test would reveal.
