...
**Fix applied:** (1) Added `realm.setDirectGrantFlow("direct grant")` to `setupRealm()` in `KeycloakTestSupport` and to `createRealm()` in `PrivilegeControllerIT`, `RoleControllerIT`, and `UserControllerIT`. (2) Added `user.setRequiredActions(List.of())` in all `createUser`/`createTestUser` helpers — Keycloak 25+ attaches default required actions to new users, which blocks ROPC with `"Account is not fully set up"`. (3) Added null-check in every `fetchToken()` with an error message that includes the full response body.
**Rule:** When creating a Keycloak realm via Admin API for ROPC testing: (a) call `realm.setDirectGrantFlow("direct grant")` before `adminClient.realms().create(realm)`; (b) call `user.setRequiredActions(List.of())` when building the `UserRepresentation` to suppress default required actions. Without (a), the token endpoint returns `access_denied`; without (b), it returns `Account is not fully set up`. In `fetchToken()`, always null-check `access_token` and include the raw response body in the exception.
