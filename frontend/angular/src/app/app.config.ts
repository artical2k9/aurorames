...
function initializeOAuth(oauthService: OAuthService, authSession: AuthSessionService) {
  return async () => {
...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    provideOAuthClient(),
    // Store OAuth tokens in localStorage so all tabs share the same session and
    // page refreshes do not clear tokens the way sessionStorage would.
    ...
    }),
    {
      ...
      deps: [OAuthService, AuthSessionService],
      multi: true,
    },
  ]
};
